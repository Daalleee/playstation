...
<head>
    ...
    <style>
        ...
        .brand-logo{width:48px;height:48px;border-radius:12px;background: #0b3d91;display:flex;align-items:center;justify-content:center;box-shadow: inset 0 0 10px rgba(255,255,255,0.07);}        
        .brand span{font-weight:600;color:#fff;font-size:18px}
        .nav-link{color: var(--muted);padding:10px 12px;border-radius:10px;display:flex;align-items:center;gap:10px}
        ...
        .card-title{color:#bfc6ff}
        .table{color: var(--text)}
        .table thead th{background:#2d3192;color:#dbe0ff;border:0}
        .table tbody tr{background: var(--panel-soft)}
        .table tbody tr+tr{border-top: 1px solid rgba(255,255,255,0.06)}
        .logout{margin-top:auto}
        a{ text-decoration:none }
        /* Responsive sidebar (off-canvas) */
        .dash-toggle{ position:fixed; left:12px; top:12px; z-index:1043; background:#4750c9; color:#fff; border:none; padding:.45rem .6rem; border-radius:.5rem; box-shadow:0 .5rem 1rem rgba(0,0,0,.25); display:none; }
        .dash-overlay{ position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:1041; display:none; }
    ...
    <div class="layout">
        <aside class="sidebar">
            ...
            <nav class="d-flex flex-column gap-1">
                <a href="{{ route('dashboard.admin') }}" class="nav-link active"><i class="bi bi-house-door"></i> Beranda</a>
                <a href="{{ route('admin.unitps.index') }}" class="nav-link"><i class="bi bi-box-seam"></i> Kelola Inventaris</a>
                <a href="{{ route('admin.pelanggan.index') }}" class="nav-link"><i class="bi bi-people"></i> Kelola Data Pelanggan</a>
                <a href="{{ route('admin.laporan') }}" class="nav-link"><i class="bi bi-graph-up"></i> Laporan</a>
