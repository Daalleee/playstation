...
class MidtransController extends Controller
{
    ...
    protected function updateRentalStatus(Rental $rental, string $transactionStatus, string $fraudStatus, $amount)
    {
        ...
        if ($transactionStatus == 'capture') {
            if ($fraudStatus == 'accept') {
                ...
                // Change UnitPS instances from 'reserved_in_cart' to 'rented' since payment is successful
                $this->updateInstanceStatusOnPaymentSuccess($rental);
    ...
    /**
     * Change UnitPS instances from 'reserved_in_cart' to 'rented' when payment is successful
     */
    protected function updateInstanceStatusOnPaymentSuccess(Rental $rental)
    {
        foreach ($rental->items as $item) {
            if ($item->rentable_type === \App\Models\UnitPS::class) {
                // Find the reserved instances for this rental item
                $reservedInstances = UnitPSInstance::where('unit_ps_id', $item->rentable_id)
                    ->where('status', 'reserved_in_cart')
                    ->limit($item->quantity)
                    ->get();

                foreach ($reservedInstances as $instance) {
                    $instance->update(['status' => 'rented']);
                }

                Log::info('UnitPS instances marked as rented after successful payment', [
                    'rental_id' => $rental->id,
                    'unit_ps_id' => $item->rentable_id,
    ...
    protected function restoreStock(Rental $rental)
    {
        foreach ($rental->items as $item) {
            if ($item->rentable) {
                ...
                if ($isUnitPS) {
                    // For UnitPS, find the reserved instances and make them available again
                    $reservedInstances = UnitPSInstance::where('unit_ps_id', $item->rentable->id)
                        ->where('status', 'reserved_in_cart')
                        ->get();
                    ...
                    // Also handle any instances that might have been marked as 'rented'
                    // (this could happen if there was a timing issue)
                    ...
                    $newStock = $item->rentable->instances()->where('status', 'available')->count();
                } else {
